...
pub fn ReverseIndex(comptime S: type, comptime T: type, comptime len: fn (state: S) usize, comptime get: fn (state: S, index: usize) T) type {
    return struct {
        state: S,
        index: usize,
        ...
        pub fn next(event: Type.Event) Type.Action {
            ...
            const i = event.index;
            if (0 < i and i <= _len(s)) {
                return Type.Action._yield(_init(s, i - 1), _get(s, i - 1));
            } else {
