...
pub fn PutAutoArrayHashMap(comptime S: type, comptime T: type, comptime U: type) type {
    return struct {
        hash_map: *AutoArrayHashMap(T, U),

        pub const Type = ConsumerType(S, @This(), Allocator.Error!void);

        pub inline fn init(hash_map: *AutoArrayHashMap(T, U)) Type.State {
        ...
        inline fn await_or_throw(hash_map: *AutoArrayHashMap(T, U), value: S) Type.Action {
